...
export const RegisterAttendanceRequestValidate = (() => {
  const _io0 = (input: any): boolean =>
    ...
    1 <= input.title.length &&
    input.title.length <= 128 &&
    'string' === typeof input.description &&
    input.description.length <= 1000 &&
    Array.isArray(input.candidateDate) &&
    input.candidateDate.every(
      (elem: any) => 'string' === typeof elem && __typia_transform__isFormatDate._isFormatDate(elem),
    );
        ...
        input.candidateDate
          .map(
            (elem: any, _index2: number) =>
              ('string' === typeof elem &&
                (__typia_transform__isFormatDate._isFormatDate(elem) ||
                  _report(_exceptionable, {
  ...
  const __is = (input: any): input is RegisterAttendanceRequest =>
    'object' === typeof input && null !== input && _io0(input);
  let errors: any;
  let _report: any;
  return (input: any): import('typia').IValidation<RegisterAttendanceRequest> => {
    if (false === __is(input)) {
      errors = [];
